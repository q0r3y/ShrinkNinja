...
function format(res, msg) {
  res.format({
    'text/plain': function () {
      res.send(msg);
    },
    'text/html': function () {
      res.send(uiResPage(msg));
    }
  });
}
  ...
  return (req, res) => {
    ...
    <script>
      ...
      async function requestCode() {
        ...
        const longUrl = window.location.pathname.slice(1)+window.location.hash;
        if (longUrl) {
          const longUrlJson = JSON.stringify({ 'longUrl' : longUrl });
          const newShortLink = await fetch('/shrink', {
            method: 'POST', body: longUrlJson,
            headers: {'Accept': 'application/json','Content-Type': 'application/json'},
          });
          newShortLink.json().then((data) => {
            $linkText.innerText = data.shortUrl || data.error;
          });
        } else {
...
module.exports = {format, error, sendWebPage, sendShortLink}
